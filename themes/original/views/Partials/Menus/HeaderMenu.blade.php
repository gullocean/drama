@foreach (isset($items) ? $items : $headerMenu['items'] as $item)
    @if ($item['type'] === 'link')
        ...
        @else
            <li><a {{ $item['visibility'] == 'admin' ? 'data-bind="visible: app.perm()"' : '' }} href="{{ $item['action'] }}">{{ $item['label'] }}</a></li>
        @endif  
    
    @elseif ($item['type'] === 'route')

        @if($options->disableNews() && $item['action'] === 'news.index')
            
        @elseif (isset($item['partial']))
            <li class="dropdown yamm-fw">
                @include('Partials.Menus.Link', array('item' => $item))
                @include('Partials.Menus.Dropdowns.'.$item['partial'])
            </li>
