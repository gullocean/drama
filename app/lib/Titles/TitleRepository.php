...
class TitleRepository extends Repository
{
     ...
     * @return Title
     */
    public function getCompleteTitle(Title $title, $provider = null)
    {
        if ( ! $provider) $provider = $this->provider;

        $provider = $provider->getFullTitle($title);
    ...
    public function needsScraping(Title $title)
    {
        ...
        if (app('mtdb.currentRequestIsFromBot')) {
            return false;
        }
        ...
        if ($title->review->isEmpty() && $date < Carbon::now()->toDateString() && ($title->updated_at && $title->updated_at->addDays(1) <= Carbon::now()))
        {
            $needs = true;
        }
        ...
        //the title
        if ($this->options->getDataProvider() == 'db' || ! $title->allow_update || (! $title->tmdb_id && ! $title->imdb_id))
        {
            return false;
        }
