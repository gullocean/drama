...
class Writer
{
	...
	private function saveImdbImages(array $images)
	{
		...
		foreach (array_slice($images, 0, 6) as $k => $v)
		{
			...
			$urls[$k] =  str_contains($img, ['imdb', 'amazon']) ? Helpers::size($img, 'original') : Helpers::original($img);
		}
	...
	private function saveImdbCastImages(array $actors)
	{
		...
		foreach ($actors as $k => $v)
		{
			if ($v['image'])
			{
				...
				$urls[$id] = str_contains($v['image'], ['imdb', 'amazon']) ? Helpers::size($v['image'], 6) : $v['image'];
			}		
		}
